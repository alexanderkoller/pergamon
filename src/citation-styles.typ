...
#let handle-missing-citation(entry, missing-citation, missing-citation-placeholder) = {
  if type(entry) == dictionary and entry.at("kind", default: none) == "pending-citation" {
    return [*?#entry.key?*] // pending-citation-placeholder(entry.key)
  }

  let key = entry
  if missing-citation == "error" {
    panic("Undefined citation key '" + key + "'.")
  } else if missing-citation == "placeholder" {
    missing-citation-placeholder(key)
  } else {
    panic("Unknown missing-citation policy '" + missing-citation + "'.")
  }
}
...
#let format-citation-alphabetic(
    ...
    /// Controls how citations to undefined bibliography keys are handled.
    /// The value `"error"` aborts with an error; the value `"placeholder"`
    /// renders `missing-citation-placeholder`.
    ///
    /// -> str
    missing-citation: "placeholder",

    /// Renders a placeholder for an undefined citation key when
    /// `missing-citation` is `"placeholder"`.
    ///
    /// -> function
    missing-citation-placeholder: key => [*?#key?*],
  ) = {
  ...
  let list-formatter(reference-dicts, form, options) = {
    ...
    let individual-citations = reference-dicts.map(x => {
      if type(x) == str {
        handle-missing-citation(x, missing-citation, missing-citation-placeholder)
      } else if type(x) == dictionary and x.at("kind", default: none) == "pending-citation" {
        handle-missing-citation(x, missing-citation, missing-citation-placeholder)
      } else {
        let lbl = x.at(0)
...
#let format-citation-authoryear(
  ...
  let list-formatter(reference-dicts, form, options) = {
    ...
    for entry in reference-dicts {
      ...
      } else if type(entry) == dictionary and entry.at("kind", default: none) == "pending-citation" {
        groups.push((author: none, authors-str: none, entries: ((label: none, entry: entry),)))
      } else {
        let lbl = entry.at(0)
    ...
    let formatted-groups = groups.map(group => {
      if group.author == none {
        ...
        handle-missing-citation(e.entry, missing-citation, missing-citation-placeholder)
